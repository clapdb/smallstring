...
TEST_CASE("smallstring algorithm compatibility") {
    SUBCASE("standard algorithms work with smallstring") {
        ...
        auto sum =
          std::accumulate(str.begin(), str.end(), 0, [](int acc, char c) { return acc + static_cast<int>(c); });
    ...
    SUBCASE("range-based algorithms") {
        ...
        auto has_vowel = std::any_of(str.begin(), str.end(),
                                     [](char c) { return c == 'a' || c == 'e' || c == 'i' || c == 'o' || c == 'u'; });
        CHECK(has_vowel);

        // Check if all characters are lowercase
        auto all_lower = std::all_of(str.begin(), str.end(), [](char c) { return c >= 'a' && c <= 'z'; });
        CHECK(all_lower);

        // None of condition
        auto no_digits = std::none_of(str.begin(), str.end(), [](char c) { return c >= '0' && c <= '9'; });
        CHECK(no_digits);
    }
