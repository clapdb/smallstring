...
TEST_CASE("smallstring substring and copy operations") {
    SUBCASE("substr comprehensive testing") {
        ...
        CHECK_THROWS_AS(
          {
              auto result = str.substr(100);
              (void)result;
          },
          std::out_of_range);
        CHECK_THROWS_AS(
          {
              auto result = str.substr(str.size() + 1);
              (void)result;
          },
          std::out_of_range);
...
TEST_CASE("smallstring replacement operations") {
    SUBCASE("replace with position and count") {
        ...
        // Basic replacement
        str.replace(6UL, 5UL, "universe");
        CHECK(str == "hello universe");

        // Replace with shorter string
        str = "hello world";
        str.replace(6UL, 5UL, "moon");
        CHECK(str == "hello moon");

        // Replace with longer string
        str = "hello world";
        str.replace(6UL, 5UL, "beautiful galaxy");
        CHECK(str == "hello beautiful galaxy");

        // Replace at beginning
        str = "hello world";
        str.replace(0UL, 5UL, "goodbye");
        CHECK(str == "goodbye world");

        // Replace at end
        str = "hello world";
        str.replace(6UL, 5UL, "friend");
        CHECK(str == "hello friend");

        // Replace entire string
        str = "hello world";
        str.replace(0UL, str.size(), "new content");
    ...
    SUBCASE("replace boundary and error conditions") {
        ...
        str.replace(0UL, 0UL, "prefix ");
        CHECK(str == "prefix test string");

        str = "test string";
        str.replace(str.size(), 0UL, " suffix");
        CHECK(str == "test string suffix");

        // Replace with count > remaining length
        str = "test string";
        str.replace(5UL, 1000UL, "replaced");
        CHECK(str == "test replaced");

        // Out of range position
        CHECK_THROWS_AS(str.replace(100UL, 1UL, "x"),
                        std::out_of_range);

        // Replace across storage boundaries
        small::small_string small_str("abc");
        small_str.replace(1UL, 1UL, "very long replacement that exceeds internal storage");
        ...
        small::small_string large(100, 'x');
        large.replace(50UL, 10UL, "replacement");
...
TEST_CASE("smallstring advanced insert operations") {
    SUBCASE("insert with string_view and custom types") {
        ...
        str.insert(6, source, 4, 10);  // "beautiful "
        CHECK(str == "hello beautiful world");
    }
    ...
    SUBCASE("insert boundary conditions and performance") {
        ...
        try {
            str.clear();
            str.insert(size_t(0), 1000000, 'M');  // Might throw or succeed
            if (str.size() == 1000000) {
                CHECK(str[0] == 'M');
                CHECK(str[999999] == 'M');
            }
